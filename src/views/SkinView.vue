    ...
    <div class="skin-container">
      ...
      <div class="skin-left">
        <div class="camera-box" @click="triggerUpload">
          ...
          <img v-else-if="photoUrl" :src="photoUrl" class="photo-preview" alt="预览"/>
        ...
        <div class="camera-btns">
          <button class="btn-outline" @click="triggerUpload">📁 上传照片</button>
          <button class="btn-outline" @click="toggleCamera">{{ cameraMode ? '📷 拍照' : '🎥 开启摄像头' }}</button>
        </div>
      ...
      <div class="skin-right">
        <div v-if="!result" class="result-placeholder">
          ...
          <p>分析结果将在这里显示</p>
          <p class="sub">上传你的照片，AI将识别肤色并推荐最适合的产品</p>
        </div>
          ...
          <div class="recommended-shades">
            ...
            <div class="shade-list">
              <div v-for="shade in result.shades" :key="shade.code" class="shade-item">
                ...
                <a :href="shade.shopeeUrl" target="_blank" class="shade-buy">购买 →</a>
              </div>
            </div>
          </div>
...
.undertone-section, .recommended-shades, .tips-section {
  background: #fff;
  border-radius: 16px;
  padding: 20px;
  box-shadow: 0 4px 20px rgba(0,0,0,0.06);
...
.undertone-section h3, .recommended-shades h3, .tips-section h3 {
  font-size: 1rem;
  font-weight: 600;
  margin-bottom: 12px;
  color: #1a1a1a;
...
.tone-map-item.active .tone-map-circle {
  box-shadow: 0 0 0 4px #d94a87, 0 4px 16px rgba(217,74,135,0.3);
}
