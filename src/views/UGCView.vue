  ...
  <div class="ugc-page">
    ...
    <div class="points-banner">
      <div class="points-item">
        ...
        <div>
          <strong>{{ t('ugc.points_upload') }}</strong>
          <span>{{ t('ugc.points_upload_val') }}</span>
        </div>
      </div>
      ...
      <div class="points-item">
        ...
        <div>
          <strong>{{ t('ugc.points_like') }}</strong>
          <span>{{ t('ugc.points_like_val') }}</span>
        </div>
      </div>
      <div class="points-divider"></div>
      <div class="points-item">
        <span class="points-icon">🎁</span>
        <div>
          <strong>{{ t('ugc.points_redeem') }}</strong>
          <span>{{ t('ugc.points_redeem_val') }}</span>
        </div>
      </div>
      <div class="points-divider"></div>
      <div class="points-item">
        <span class="points-icon">👑</span>
        <div>
          <strong>{{ t('ugc.points_mine') }}</strong>
          <span class="my-points">{{ myPoints }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="upload-section">
      <div class="upload-box" @click="triggerUpload" @dragover.prevent @drop.prevent="onDrop">
        <div v-if="!uploadPreview" class="upload-inner">
          ...
          <p>{{ t('ugc.upload_tip') }}</p>
          <span>{{ t('ugc.upload_sub') }}</span>
        </div>
        <img v-else :src="uploadPreview" class="upload-preview" alt="preview"/>
      </div>
      <input ref="fileInput" type="file" accept="image/*" style="display:none" @change="onFileChange"/>

      <div v-if="uploadPreview" class="upload-form">
        <input v-model="caption" class="caption-input" :placeholder="t('ugc.caption_placeholder')"/>
        <div class="tag-select">
          <span class="tag-label">{{ t('ugc.style_label') }}</span>
    ...
    <div v-if="showSuccess" class="success-toast">{{ t('ugc.success') }}</div>
...
async function submitPost() {
  ...
  posts.value.unshift({
    ...
    likes: 0, liked: false, shade: null, shadeColor: null,
  })
  myPoints.value += 50
...
function toggleLike(post) {
  ...
  else { post.likes++; post.liked = true; myPoints.value += 10 }
}
...
<style scoped>
.ugc-page { max-width: 1100px; margin: 0 auto; padding: 48px 24px; }
...
.points-banner {
  ...
  border-radius: 20px; padding: 24px 32px; margin-bottom: 40px; color: #fff; flex-wrap: wrap; gap: 16px;
}
...
.upload-box {
  border: 2px dashed #d94a87; border-radius: 20px; min-height: 180px;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; background: #fdf0f6; overflow: hidden; transition: all 0.2s; margin-bottom: 16px;
}
...
.caption-input { width: 100%; padding: 12px 16px; border: 1.5px solid #e0e0e0; border-radius: 12px; font-size: 0.95rem; outline: none; transition: border-color 0.2s; }
...
.style-tag { padding: 6px 14px; border: 1.5px solid #e0e0e0; border-radius: 20px; background: transparent; font-size: 0.82rem; cursor: pointer; color: #666; transition: all 0.2s; }
.style-tag.active { background: #d94a87; border-color: #d94a87; color: #fff; }
.submit-btn { width: 100%; padding: 14px; background: #d94a87; color: #fff; border: none; border-radius: 12px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
.submit-btn:hover:not(:disabled) { background: #c03878; }
.submit-btn:disabled { opacity: 0.6; cursor: not-allowed; }

.filter-bar { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 24px; }
.filter-btn { padding: 8px 18px; border: 1.5px solid #e0e0e0; border-radius: 20px; background: transparent; font-size: 0.88rem; cursor: pointer; color: #666; transition: all 0.2s; }
.filter-btn.active { background: #d94a87; border-color: #d94a87; color: #fff; }

.ugc-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
.ugc-card { background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 16px rgba(0,0,0,0.06); transition: transform 0.3s; }
...
.ugc-overlay { position: absolute; bottom: 0; left: 0; right: 0; padding: 12px; background: linear-gradient(transparent, rgba(0,0,0,0.4)); display: flex; justify-content: flex-end; }
.like-btn { background: rgba(255,255,255,0.9); border: none; border-radius: 20px; padding: 6px 12px; font-size: 0.85rem; cursor: pointer; font-weight: 600; transition: all 0.2s; }
.like-btn:hover { background: #fff; transform: scale(1.1); }
.ugc-info { padding: 16px; }
.ugc-user { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.avatar { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 0.8rem; font-weight: 700; }
...
.ugc-tag { background: #fce4f0; color: #d94a87; padding: 2px 8px; border-radius: 10px; font-size: 0.75rem; }
.ugc-shade { display: flex; align-items: center; gap: 6px; font-size: 0.8rem; color: #888; }
.shade-dot { width: 14px; height: 14px; border-radius: 50%; box-shadow: 0 1px 4px rgba(0,0,0,0.15); }

.success-toast {
  position: fixed; bottom: 32px; left: 50%; transform: translateX(-50%);
  background: #1a1a1a; color: #fff; padding: 14px 28px; border-radius: 50px;
  font-size: 0.95rem; font-weight: 500; z-index: 999; animation: slideUp 0.3s ease;
}
@keyframes slideUp {
...
@media (max-width: 768px) {
  .ugc-page { padding: 24px 16px; }
  .page-header h1 { font-size: 1.6rem; }
  .points-banner { padding: 20px 16px; gap: 12px; }
  .points-divider { display: none; }
